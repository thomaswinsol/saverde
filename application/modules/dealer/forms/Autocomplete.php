<?php
class dealer_Form_Autocomplete extends My_Form
{
    ...
    public function init()
    {
        if ( !empty($this->_productid) ) {
            ...
            $location='/dealer/product/selecteer/productid/'.$this->_productid.'/productdetail/'.$this->_productdetail.'/id/';
            if (trim($this->_productdetail)=="F") {
                $label = "lblFoto";
                $source  ='/dealer/foto/autocomplete';
                $elem = new ZendX_JQuery_Form_Element_AutoComplete("AutocompleteF", array('label' => $label, 'size'=>30 , 'maxlength'=>8));
            }
            else {
                $label = "lblCategorie";
                $source  ='/dealer/categorie/autocomplete';
                $elem = new ZendX_JQuery_Form_Element_AutoComplete("AutocompleteC", array('label' => $label, 'size'=>30 , 'maxlength'=>8));
            }
        }
        else {
            $source  ='/dealer/'. trim($this->_controller). '/autocomplete';
            $location='/dealer/'. trim($this->_controller). '/detail/id/';
